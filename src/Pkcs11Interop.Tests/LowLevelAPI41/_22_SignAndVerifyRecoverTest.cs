    ...
    public class _22_SignAndVerifyRecoverTest
    {
        ...
        public void _01_BasicSignAndVerifyRecoverTest()
        {
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11Library.C_SignRecover(session, sourceData, ConvertUtils.UInt32FromInt32(sourceData.Length), null, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11Library.C_SignRecover(session, sourceData, ConvertUtils.UInt32FromInt32(sourceData.Length), signature, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11Library.C_VerifyRecover(session, signature, ConvertUtils.UInt32FromInt32(signature.Length), null, ref recoveredDataLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11Library.C_VerifyRecover(session, signature, ConvertUtils.UInt32FromInt32(signature.Length), recoveredData, ref recoveredDataLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
