        ...
        public void _01_BasicInitTokenAndPinTest()
        {
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                if ((tokenInfo.Flags & CKF.CKF_TOKEN_INITIALIZED) != CKF.CKF_TOKEN_INITIALIZED)
                {
                    ...
                    rv = pkcs11Library.C_InitToken(slotId, Settings.SecurityOfficerPinArray, ConvertUtils.UInt64FromInt32(Settings.SecurityOfficerPinArray.Length), label);
                    if (rv != CKR.CKR_OK)
                        Assert.Fail(rv.ToString());
                    ...
                    // Initialize user pin
                    rv = pkcs11Library.C_InitPIN(session, Settings.NormalUserPinArray, ConvertUtils.UInt64FromInt32(Settings.NormalUserPinArray.Length));
                    if (rv != CKR.CKR_OK)
                        Assert.Fail(rv.ToString());
