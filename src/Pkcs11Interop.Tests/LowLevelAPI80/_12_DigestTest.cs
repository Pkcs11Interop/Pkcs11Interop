    ...
    public class _12_DigestTest
    {
        ...
        public void _01_DigestSinglePartTest()
        {
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11Library.C_Digest(session, sourceData, ConvertUtils.UInt64FromInt32(sourceData.Length), null, ref digestLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11Library.C_Digest(session, sourceData, ConvertUtils.UInt64FromInt32(sourceData.Length), digest, ref digestLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                using (MemoryStream inputStream = new MemoryStream(sourceData))
                {
                    ...
                    while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
                    {
                        ...
                        rv = pkcs11Library.C_DigestUpdate(session, part, ConvertUtils.UInt64FromInt32(bytesRead));
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
                    }
