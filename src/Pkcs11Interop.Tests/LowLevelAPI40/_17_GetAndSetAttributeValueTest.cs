    ...
    public class _17_GetAndSetAttributeValueTest
    {
        ...
        public void _01_GetAttributeValueTest()
        {
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11Library.C_GetAttributeValue(session, objectId, template, ConvertUtils.UInt32FromInt32(template.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
        ...
        public void _02_SetAttributeValueTest()
        {
            ...
            using (Pkcs11Library pkcs11Library = new Pkcs11Library(Settings.Pkcs11LibraryPath))
            {
                ...
                // Set attributes
                rv = pkcs11Library.C_SetAttributeValue(session, objectId, template, ConvertUtils.UInt32FromInt32(template.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
