...
namespace Net.Pkcs11Interop.Tests.LowLevelAPI40
{
    ...
    public partial class _28_Pkcs11UriUtilsTest
    {
        ...
        public void _06_GetMatchingSlotList()
        {
            ...
            {
                CKR rv = pkcs11Library.C_Initialize(Settings.InitArgs40);
        ...
        public void _07_GetObjectAttributes()
        {
            ...
            Assert.IsTrue(attributes[0].type == ConvertUtils.UInt32FromCKA(CKA.CKA_CLASS));
            NativeULong ckaClass = 0;
            CkaUtils.ConvertValue(ref attributes[0], out ckaClass);
            Assert.IsTrue(ckaClass == ConvertUtils.UInt32FromCKO(CKO.CKO_PRIVATE_KEY));

            Assert.IsTrue(attributes[1].type == ConvertUtils.UInt32FromCKA(CKA.CKA_LABEL));
            string ckaLabel = null;
            CkaUtils.ConvertValue(ref attributes[1], out ckaLabel);
            Assert.IsTrue(ckaLabel == "foo");

            Assert.IsTrue(attributes[2].type == ConvertUtils.UInt32FromCKA(CKA.CKA_ID));
            byte[] ckaId = null;
