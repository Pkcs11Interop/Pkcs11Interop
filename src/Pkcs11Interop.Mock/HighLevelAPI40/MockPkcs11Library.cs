...
namespace Net.Pkcs11Interop.Mock.HighLevelAPI40
{
    ...
    /// </summary>
    public class MockPkcs11Library : Net.Pkcs11Interop.HighLevelAPI40.Pkcs11Library, IMockPkcs11Library
    {
        /// <summary>
            ...
            : base(factories, libraryPath)
        {
            ...
            try
            {
                ...
                _p11 = new LowLevelAPI40.MockPkcs11Library(libraryPath);
                Initialize(appType);
            }
            catch
            {
            ...
            : base(factories, libraryPath)
        {
            ...
            {
                _logger.Info("Loading PKCS#11 library {0}", _libraryPath);
                _p11 = new LowLevelAPI40.MockPkcs11Library(libraryPath, (initType == InitType.WithFunctionList));
                Initialize(appType);
            }
            catch
            {
        ...
        public List<ulong> GetUnmanagedStructSizeList()
        {
            ...
            CKR rv = ((LowLevelAPI40.MockPkcs11Library)_p11).C_GetUnmanagedStructSizeList(null, ref sizeCount);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_GetUnmanagedStructSizeList", rv);

            NativeULong[] sizeList = new NativeULong[sizeCount];
            rv = ((LowLevelAPI40.MockPkcs11Library)_p11).C_GetUnmanagedStructSizeList(sizeList, ref sizeCount);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_GetUnmanagedStructSizeList", rv);
