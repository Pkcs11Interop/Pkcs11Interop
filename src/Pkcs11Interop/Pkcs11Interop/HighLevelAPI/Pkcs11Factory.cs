...
namespace Net.Pkcs11Interop.HighLevelAPI
{
    ...
    public class Pkcs11Factory : IPkcs11Factory
    {
        ...
        public Pkcs11Factory()
        {
            if (Platform.UnmanagedLongSize == 4)
            {
                if (Platform.StructPackingSize == 0)
                    _factory = new HighLevelAPI40.Pkcs11Factory();
                else
                    _factory = new HighLevelAPI41.Pkcs11Factory();
            }
            else
            {
                if (Platform.StructPackingSize == 0)
                    _factory = new HighLevelAPI80.Pkcs11Factory();
                else
                    _factory = new HighLevelAPI81.Pkcs11Factory();
            }
        }
        ...
        public IPkcs11 CreatePkcs11(string libraryPath, AppType appType)
        {
            return _factory.CreatePkcs11(libraryPath, appType);
        }
        ...
        /// <returns>High level PKCS#11 wrapper</returns>
        public IPkcs11 CreatePkcs11(string libraryPath, AppType appType, InitType initType)
        {
            return _factory.CreatePkcs11(libraryPath, appType, initType);
        }
    }
}
