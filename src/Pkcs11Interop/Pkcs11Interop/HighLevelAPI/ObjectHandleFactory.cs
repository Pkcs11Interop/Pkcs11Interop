        ...
        public ObjectHandleFactory()
        {
            if (Platform.UnmanagedLongSize == 4)
            {
                if (Platform.StructPackingSize == 0)
                    _factory = new HighLevelAPI40.ObjectHandleFactory();
                else
                    _factory = new HighLevelAPI41.ObjectHandleFactory();
            }
            else
            {
                if (Platform.StructPackingSize == 0)
                    _factory = new HighLevelAPI80.ObjectHandleFactory();
                else
                    _factory = new HighLevelAPI81.ObjectHandleFactory();
            }
        }
