    ...
    public class CkTlsPrfParams : ICkTlsPrfParams
    {
        ...
        public byte[] Output
        {
            get
            {
                ...
                NativeULong outputLen = NativeULongUtils.ConvertUInt64FromByteArray(outputLenBytes);
                return UnmanagedMemory.Read(_lowLevelStruct.Output, NativeULongUtils.ConvertUInt64ToInt32(outputLen));
            }
        }
        ...
        public CkTlsPrfParams(byte[] seed, byte[] label, NativeULong outputLen)
        {
            ...
            if (label != null)
            {
                ...
                _lowLevelStruct.LabelLen = NativeULongUtils.ConvertUInt64FromInt32(label.Length);
            }

            if (outputLen < 1)
                throw new ArgumentException("Value has to be positive number", "outputLen");

            _lowLevelStruct.Output = UnmanagedMemory.Allocate(NativeULongUtils.ConvertUInt64ToInt32(outputLen));

            byte[] outputLenBytes = NativeULongUtils.ConvertUInt64ToByteArray(outputLen);
