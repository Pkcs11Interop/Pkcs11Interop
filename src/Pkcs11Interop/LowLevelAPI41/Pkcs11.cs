...
namespace Net.Pkcs11Interop.LowLevelAPI41
{
    ...
    public class Pkcs11 : IDisposable
    {
        ...
        public CKR C_GetUnmanagedStructSizeList(uint[] sizeList, ref uint count)
        {
            if (this._disposed)
                throw new ObjectDisposedException(this.GetType().FullName);
        ...
        /// <param name="count">Location that receives the number of sizes</param>
        /// <returns>CKR_ARGUMENTS_BAD, CKR_BUFFER_TOO_SMALL, CKR_OK</returns>
        public CKR C_GetManagedStructSizeList(uint[] sizeList, ref uint count)
        {
            int[] sizes = new int[] {
            ...
            uint sizes_count = Convert.ToUInt32(sizes.Length);
            ...
            else
            {
                ...
                for (int i = 0; i < sizes_count; i++)
                    sizeList[i] = Convert.ToUInt32(sizes[i]);
