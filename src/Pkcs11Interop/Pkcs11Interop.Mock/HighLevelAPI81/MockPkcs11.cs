...
namespace Net.Pkcs11Interop.Mock.HighLevelAPI81
{
    ...
    /// </summary>
    public class MockPkcs11 : Net.Pkcs11Interop.HighLevelAPI81.Pkcs11, IMockPkcs11
    {
        /// <summary>
            ...
            : base(factories)
        {
            try
            {
                _p11 = new LowLevelAPI81.MockPkcs11(libraryPath);
                Initialize(appType);
            }
            catch
            {
            ...
            : base(factories)
        {
            try
            {
                _p11 = new LowLevelAPI81.MockPkcs11(libraryPath, (initType == InitType.WithFunctionList));
                Initialize(appType);
            }
            catch
            {
        ...
        public List<ulong> GetUnmanagedStructSizeList()
        {
            ...
            CKR rv = ((LowLevelAPI81.MockPkcs11)_p11).C_GetUnmanagedStructSizeList(null, ref sizeCount);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_GetUnmanagedStructSizeList", rv);

            NativeULong[] sizeList = new NativeULong[sizeCount];
            rv = ((LowLevelAPI81.MockPkcs11)_p11).C_GetUnmanagedStructSizeList(sizeList, ref sizeCount);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_GetUnmanagedStructSizeList", rv);
