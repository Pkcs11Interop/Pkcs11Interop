    ...
    public class _12_DigestTest
    {
        ...
        public void _01_DigestSinglePartTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11.C_Digest(session, sourceData, NativeULongUtils.ConvertUInt32FromInt32(sourceData.Length), null, ref digestLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_Digest(session, sourceData, NativeULongUtils.ConvertUInt32FromInt32(sourceData.Length), digest, ref digestLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                using (MemoryStream inputStream = new MemoryStream(sourceData))
                {
                    ...
                    while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
                    {
                        ...
                        rv = pkcs11.C_DigestUpdate(session, part, NativeULongUtils.ConvertUInt32FromInt32(bytesRead));
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
                    }
