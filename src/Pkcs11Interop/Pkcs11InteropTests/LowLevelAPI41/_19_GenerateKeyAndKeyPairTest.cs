    ...
    public class _19_GenerateKeyAndKeyPairTest
    {
        ...
        public void _01_GenerateKeyTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11.C_GenerateKey(session, ref mechanism, template, NativeULongUtils.ConvertUInt32FromInt32(template.Length), ref keyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
        ...
        public void _02_GenerateKeyPairTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11.C_GenerateRandom(session, ckaId, NativeULongUtils.ConvertUInt32FromInt32(ckaId.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_GenerateKeyPair(session, ref mechanism, pubKeyTemplate, NativeULongUtils.ConvertUInt32FromInt32(pubKeyTemplate.Length), privKeyTemplate, NativeULongUtils.ConvertUInt32FromInt32(privKeyTemplate.Length), ref pubKeyId, ref privKeyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
