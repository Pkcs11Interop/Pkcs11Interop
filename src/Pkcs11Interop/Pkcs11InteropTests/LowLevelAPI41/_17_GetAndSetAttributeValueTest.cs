    ...
    public class _17_GetAndSetAttributeValueTest
    {
        ...
        public void _01_GetAttributeValueTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11.C_GetAttributeValue(session, objectId, template, NativeULongUtils.GetUInt32FromInt32(template.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());

                // In LowLevelAPI we have to allocate unmanaged memory for attribute value
                for (int i = 0; i < template.Length; i++)
                    template[i].value = UnmanagedMemory.Allocate(NativeULongUtils.PutUInt32ToInt32(template[i].valueLen));
                ...
                byte[] ckaLabel = UnmanagedMemory.Read(template[0].value, NativeULongUtils.PutUInt32ToInt32(template[0].valueLen));
        ...
        public void _02_SetAttributeValueTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                // Set attributes
                rv = pkcs11.C_SetAttributeValue(session, objectId, template, NativeULongUtils.GetUInt32FromInt32(template.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
