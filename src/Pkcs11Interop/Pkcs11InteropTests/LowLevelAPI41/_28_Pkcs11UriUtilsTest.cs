    ...
    public partial class _28_Pkcs11UriUtilsTest
    {
        ...
        private byte[] SignData(byte[] data, string uri)
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(pkcs11Uri.ModulePath, true))
            {
                ...
                rv = pkcs11.C_Login(session, CKU.CKU_USER, pinValue, NativeULongUtils.GetUInt32FromInt32(pinValue.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_FindObjectsInit(session, attributes, NativeULongUtils.GetUInt32FromInt32(attributes.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_Sign(session, data, NativeULongUtils.GetUInt32FromInt32(data.Length), null, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_Sign(session, data, NativeULongUtils.GetUInt32FromInt32(data.Length), signature, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
