    ...
    public class _21_SignAndVerifyTest
    {
        ...
        public void _01_SignAndVerifySinglePartTest()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = pkcs11.C_Sign(session, sourceData, NativeULongUtils.GetUInt64FromInt32(sourceData.Length), null, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = pkcs11.C_Sign(session, sourceData, NativeULongUtils.GetUInt64FromInt32(sourceData.Length), signature, ref signatureLen);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                // Verify signature
                rv = pkcs11.C_Verify(session, sourceData, NativeULongUtils.GetUInt64FromInt32(sourceData.Length), signature, NativeULongUtils.GetUInt64FromInt32(signature.Length));
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                using (MemoryStream inputStream = new MemoryStream(sourceData))
                {
                    ...
                    while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
                    {
                        ...
                        rv = pkcs11.C_SignUpdate(session, part, NativeULongUtils.GetUInt64FromInt32(bytesRead));
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
                    }
                ...
                using (MemoryStream inputStream = new MemoryStream(sourceData))
                {
                    ...
                    {
                        // Process each individual source data part
                        rv = pkcs11.C_VerifyUpdate(session, part, NativeULongUtils.GetUInt64FromInt32(bytesRead));
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
                    }
