            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                using (MemoryStream inputStream = new MemoryStream(sourceData), outputStream = new MemoryStream())
                {
                    ...
                    while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
                    {
                        ...
                        rv = pkcs11.C_DigestEncryptUpdate(session, part, NativeULongUtils.ConvertUInt64FromInt32(bytesRead), encryptedPart, ref encryptedPartLen);
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
                ...
                using (MemoryStream inputStream = new MemoryStream(encryptedData), outputStream = new MemoryStream())
                {
                    ...
                    while ((bytesRead = inputStream.Read(encryptedPart, 0, encryptedPart.Length)) > 0)
                    {
                        ...
                        rv = pkcs11.C_DecryptDigestUpdate(session, encryptedPart, NativeULongUtils.ConvertUInt64FromInt32(bytesRead), part, ref partLen);
                        if (rv != CKR.CKR_OK)
                            Assert.Fail(rv.ToString());
