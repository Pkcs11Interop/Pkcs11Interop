...
namespace Net.Pkcs11Interop.HighLevelAPI81.MechanismParams
{
    ...
    public class CkCmsSigParams : IMechanismParams, IDisposable
    {
        ...
        /// </summary>
        private LowLevelAPI81.MechanismParams.CK_CMS_SIG_PARAMS _lowLevelStruct = new LowLevelAPI81.MechanismParams.CK_CMS_SIG_PARAMS();
        ...
        public CkCmsSigParams(ObjectHandle certificateHandle, ulong? signingMechanism, ulong? digestMechanism, string contentType, byte[] requestedAttributes, byte[] requiredAttributes)
        {
            ...
            {
                byte[] bytes = Common.ConvertUtils.ULongToBytes(signingMechanism.Value);
            ...
            {
                byte[] bytes = Common.ConvertUtils.ULongToBytes(digestMechanism.Value);
