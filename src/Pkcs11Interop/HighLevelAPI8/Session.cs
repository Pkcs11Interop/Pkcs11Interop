...
namespace Net.Pkcs11Interop.HighLevelAPI8
{
    ...
    public class Session : IDisposable
    {
        ...
        internal Session(LowLevelAPI8.Pkcs11 pkcs11, ulong sessionId)
        {
            if (pkcs11 == null)
        ...
        public SessionInfo GetSessionInfo()
        {
            ...
            LowLevelAPI8.CK_SESSION_INFO sessionInfo = new LowLevelAPI8.CK_SESSION_INFO();
        ...
        public ObjectHandle CreateObject(List<ObjectAttribute> attributes)
        {
            ...
            LowLevelAPI8.CK_ATTRIBUTE[] template = null;
            ulong templateLength = 0;
            
            if (attributes != null)
            {
                templateLength = Convert.ToUInt64(attributes.Count);
                template = new LowLevelAPI8.CK_ATTRIBUTE[templateLength];
        ...
        public List<ObjectAttribute> GetAttributeValue(ObjectHandle objectHandle, List<ulong> attributes)
        {
            ...
            LowLevelAPI8.CK_ATTRIBUTE[] template = new LowLevelAPI8.CK_ATTRIBUTE[attributes.Count];
            for (int i = 0; i < attributes.Count; i++)
                template[i] = LowLevelAPI8.CkaUtils.CreateAttribute(attributes[i]);
            ...
            _p11.C_GetAttributeValue(_sessionId, objectHandle.ObjectId, template, Convert.ToUInt64(template.Length));
            if ((rv != CKR.CKR_OK) && (rv != CKR.CKR_ATTRIBUTE_SENSITIVE) && (rv != CKR.CKR_ATTRIBUTE_TYPE_INVALID))
                throw new Pkcs11Exception("C_GetAttributeValue", rv);
        ...
        public byte[] Encrypt(Mechanism mechanism, ObjectHandle keyHandle, byte[] data)
        {
            ...
            LowLevelAPI8.CK_MECHANISM ckMechanism = mechanism.CkMechanism;
        ...
        public byte[] Digest(Mechanism mechanism, byte[] data)
        {
            ...
            ulong digestLen = 0;
            rv = _p11.C_Digest(_sessionId, data, Convert.ToUInt64(data.Length), null, ref digestLen);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_Digest", rv);

            byte[] digest = new byte[digestLen];
            rv = _p11.C_Digest(_sessionId, data, Convert.ToUInt64(data.Length), digest, ref digestLen);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_Digest", rv);
        ...
        public byte[] Digest(Mechanism mechanism, Stream inputStream, int bufferLength)
        {
            ...
            while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
            {
                rv = _p11.C_DigestUpdate(_sessionId, part, Convert.ToUInt64(bytesRead));
                if (rv != CKR.CKR_OK)
        ...
        public byte[] Sign(Mechanism mechanism, ObjectHandle keyHandle, byte[] data)
        {
            ...
            rv = _p11.C_Sign(_sessionId, data, Convert.ToUInt64(data.Length), null, ref signatureLen);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_Sign", rv);

            byte[] signature = new byte[signatureLen];
            rv = _p11.C_Sign(_sessionId, data, Convert.ToUInt64(data.Length), signature, ref signatureLen);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_Sign", rv);
        ...
        public void Verify(Mechanism mechanism, ObjectHandle keyHandle, Stream inputStream, byte[] signature, out bool isValid, int bufferLength)
        {
            ...
            while ((bytesRead = inputStream.Read(part, 0, part.Length)) > 0)
            {
                rv = _p11.C_VerifyUpdate(_sessionId, part, Convert.ToUInt64(bytesRead));
                if (rv != CKR.CKR_OK)
        ...
        public byte[] DigestEncrypt(Mechanism digestingMechanism, Mechanism encryptionMechanism, ObjectHandle keyHandle, Stream inputStream, Stream outputStream, int bufferLength)
        {
            ...
            LowLevelAPI8.CK_MECHANISM ckDigestingMechanism = digestingMechanism.CkMechanism;

            CKR rv = _p11.C_DigestInit(_sessionId, ref ckDigestingMechanism);
            if (rv != CKR.CKR_OK)
                throw new Pkcs11Exception("C_DigestInit", rv);

            LowLevelAPI8.CK_MECHANISM ckEncryptionMechanism = encryptionMechanism.CkMechanism;
        ...
        public byte[] DecryptDigest(Mechanism digestingMechanism, Mechanism decryptionMechanism, ObjectHandle keyHandle, Stream inputStream, Stream outputStream, int bufferLength)
        {
            ...
            LowLevelAPI8.CK_MECHANISM ckDecryptionMechanism = decryptionMechanism.CkMechanism;
        ...
        public byte[] SignEncrypt(Mechanism signingMechanism, ObjectHandle signingKeyHandle, Mechanism encryptionMechanism, ObjectHandle encryptionKeyHandle, Stream inputStream, Stream outputStream, int bufferLength)
        {
            ...
            LowLevelAPI8.CK_MECHANISM ckSigningMechanism = signingMechanism.CkMechanism;
        ...
        public void DecryptVerify(Mechanism verificationMechanism, ObjectHandle verificationKeyHandle, Mechanism decryptionMechanism, ObjectHandle decryptionKeyHandle, Stream inputStream, Stream outputStream, byte[] signature, out bool isValid, int bufferLength)
        {
            ...
            LowLevelAPI8.CK_MECHANISM ckVerificationMechanism = verificationMechanism.CkMechanism;
        ...
        public void GenerateKeyPair(Mechanism mechanism, List<ObjectAttribute> publicKeyAttributes, List<ObjectAttribute> privateKeyAttributes, out ObjectHandle publicKeyHandle, out ObjectHandle privateKeyHandle)
        {
            ...
            LowLevelAPI8.CK_ATTRIBUTE[] publicKeyTemplate = null;
            ...
            if (publicKeyAttributes != null)
            {
                ...
                publicKeyTemplate = new LowLevelAPI8.CK_ATTRIBUTE[publicKeyTemplateLength];
                for (int i = 0; i < Convert.ToInt32(publicKeyTemplateLength); i++)
                    publicKeyTemplate[i] = publicKeyAttributes[i].CkAttribute;
            }

            LowLevelAPI8.CK_ATTRIBUTE[] privateKeyTemplate = null;
            ...
            if (privateKeyAttributes != null)
            {
                ...
                privateKeyTemplate = new LowLevelAPI8.CK_ATTRIBUTE[privateKeyTemplateLength];
        ...
        public ObjectHandle UnwrapKey(Mechanism mechanism, ObjectHandle unwrappingKeyHandle, byte[] wrappedKey, List<ObjectAttribute> attributes)
        {
            ...
            if (attributes != null)
            {
                template = new LowLevelAPI8.CK_ATTRIBUTE[attributes.Count];
                for (int i = 0; i < attributes.Count; i++)
                    template[i] = attributes[i].CkAttribute;
